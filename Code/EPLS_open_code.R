...
#                                      0. Required packages  
#======================================================================================================#
library("devtools");library("dplyr");library("MASS");library("data.table");library("xtable")
library("copula");library("VineCopula");library("fExtremes");library("RMThreshold")
library("EnvStats");library("graphics");library("rgl");
library("matrixcalc");library("ginv");require("VGAM")
require("ggplot2");require("geometry");require("compositions")
...
#__________________________1.1 Basic functions_________________________________________________________#
...
#__________________________1.2 Extreme-PLS function________________________________________________________#

######## Description
#This function returns the beta estimate by Extreme-PLS approach.
######## Inputs
...
######## Outputs
#Beta estimate vector 

######## Code Extreme-PLS estimator
epls<-function(X,Y,y){ 
  bool<-Y>y
  X.bool<-X[bool,]
  Y.bool<-Y[bool]  
...
#__________________________1.3 Simulation of X and Y function ________________________________________________________#

######## Description
#X_Y_simu: this function returns a list containing the vector of simulated Y and simulated X 
#according to the M1 model: X=g(Y)beta+epsilon, where g(y)=y^c ans epsilon the noise~N(0,sigma^2).
...
######## Outputs
#The first index of the list returns the simulated Y, then the other indices return the 
#simulated X according to the link function. For example if the size of the power vector is 4,
#that is to say we will have 4 X matrices. See example below
...
X_Y_simu<-function(n,p,beta,power,sigma,r,dist,dist.param,copula.fam,copula.param){
    #Simulation of Y
    if(dist=="pareto"){Y<-pareto.simu(dist.param[1],dist.param[2],n)} #Pareto distribution
    if(dist=="student"){Y<-abs(rt(n,param.dist[1]))}                  #Student distribution
    List_X<-list()
    List_X[[1]]<-Y
    ...
    #Normalization of Y
      ...
      if(copula.fam==5){obj <- BiCop(family = copula.fam, copula.param)} #Frank or Normal copula
...
#copula. fam : 1 = Gaussian copula, 5=Frank copula
...
#__________________________2.2 Simulation process ________________________________________________________#

#Cos: results storage table containing: simu = number of replications, k=number of exceedances, 
#cos^2 for each link function power
cos<-data.table(simu=numeric(),k=numeric(),cos2.x.1.5=numeric(),cos2.x=numeric(),cos2.x.0.5=numeric(),
                    cos2.x.0.25=numeric())
...
for(j in 1:N){ 
  ...
  for(i in k.threshold:length(Y) ){
      ...
      for(l in 1:length(c)){ 
        ...
        #saveRDS(cos,"/scratch/mbouseba/Documents/PhD/PLS/Etude PLS extreme/Simulations/Replications/EPLS&NormalCopula/EPLS_Normal_Pareto_03_00.rds")
  }
}
...
cos.rep<-as.data.frame(cos.rep)

####### Graphical representation
#png("/scratch/mbouseba/Documents/PhD/PLS/Etude PLS extreme/Simulations/Replications/EPLS&NormalCopula/EPLS_Normal_Pareto_03_00.png", width = 6, height = 5, units='in', res = 300,bg = "transparent")
...
Insight.data <- function(data) {
  require("data.table")
  
  setDT(data)
  Insight_data_list <- list()
  
  # Features
  Insight_data_list$Features <- colnames(data)
  
  # N_rows
  Insight_data_list$N_rows <- nrow(data)
  
  # Class
  Insight_data_list$Class <- sapply(data,function(x) class(x))
  
  # Unique_val
  Insight_data_list$Unique_val <- sapply(data,function(x) uniqueN(x))
  
  # NAs
  Insight_data_list$NAs <- sapply(data,function(x) sum(is.na(x)))
  Insight_data_list$NAs_rate <- sapply(data,function(x) 100*round(sum(is.na(x))/Insight_data_list$N_rows,4))
  
  # NULL
  Insight_data_list$NULLs <- sapply(data,function(x) sum(is.null(x)))
  Insight_data_list$NULLs_rate <- sapply(data,function(x) 100*round(sum(is.null(x))/Insight_data_list$N_rows,4))
  
  # Blank
  Insight_data_list$Blanks <- sapply(data,function(x) sum(x=="",na.rm = T))
  Insight_data_list$Blanks_rate <- sapply(data,function(x) 100*round(sum(x=="",na.rm = T)/Insight_data_list$N_rows,4))
  
  # Mean
  Insight_data_list$Mean <- sapply(data,function(x) if (is.numeric(x)) {mean(x,na.rm = T)} else {NA})
  
  # Standard dev
  Insight_data_list$Sd <- sapply(data,function(x) if (is.numeric(x)) {sd(x,na.rm = T)} else {NA})
  
  # Variance
  Insight_data_list$Var <- sapply(data,function(x) if (is.numeric(x)) {var(x,na.rm = T)} else {NA})
  
  # Mediane
  Insight_data_list$Median <- sapply(data,function(x) if (is.numeric(x)) {
    quantile(x,probs = 0.50,na.rm = T)
  } else {NA})
  
  # Q1
  Insight_data_list$Q1 <- sapply(data,function(x) if (is.numeric(x)) {
    quantile(x,probs = 0.25,na.rm = T)
  } else {NA})
  
  # Q3
  Insight_data_list$Q3 <- sapply(data,function(x) if (is.numeric(x)) {
    quantile(x,probs = 0.75,na.rm = T)
  } else {NA})
  
  # Min
  Insight_data_list$Min <- sapply(data,function(x) if (is.numeric(x)) {
    min(x,na.rm = T)
  } else {NA})
  
  # Max
  Insight_data_list$Max <- sapply(data,function(x) if (is.numeric(x)) {
    max(x,na.rm = T)
  } else {NA})
  
  # Outliers
  Insight_data_list$Outliers <- sapply(data,function(x) if (is.numeric(x)) {
    sum((x > mean(x,na.rm = T)+qnorm(0.975)*sd(x,na.rm = T)) 
        | (x < mean(x,na.rm = T)-qnorm(0.975)*sd(x,na.rm = T)),na.rm = T)
  } else {NA})
  
  # Features_EoF
  Insight_data_list$Features_EoF <- Insight_data_list$Features
  
  Res <- do.call(cbind.data.frame,Insight_data_list)
  setDT(Res)
  return(Res)
}

#### Data description
Insight.data(data.selec)
...
#png("/scratch/mbouseba/Documents/PhD/PLS/Etude PLS extreme/RICA/Inverse_yield/Chosen model/hist_2014.png", width = 6, height = 5, units='in', res = 300,bg = "transparent")
...
n<-length(Y)
k<-150
#png("/scratch/mbouseba/Documents/PhD/PLS/Etude PLS extreme/RICA/Inverse_yield/Chosen model/qqplot_2014_150_2.png", width = 6, height = 5, units='in', res = 300,bg = "transparent")
qqplt(Y,k,length(Y))
#dev.off()
...
#### Hill plot
#png("/scratch/mbouseba/Documents/PhD/PLS/Etude PLS extreme/RICA/Inverse_yield/Chosen model/hillplot_2014_150.png", width = 6, height = 5, units='in', res = 300,bg = "transparent")
...
#png("/scratch/mbouseba/Documents/PhD/PLS/Etude PLS extreme/RICA/Inverse_yield/Chosen model/corXwX_2014_150_2.png", width = 7, height = 5, units='in', res = 300,bg = "transparent") #width = 6, height = 5
...
#png("/scratch/mbouseba/Documents/PhD/PLS/Etude PLS extreme/RICA/corRwY_2014_150.png", width = 7, height = 5, units='in', res = 300,bg = "transparent") #width = 6, height = 5
par(mar=c(5.1, 2.1, 4.1, 10.1), xpd=FALSE)
...
plot(1:11,filt[class==2 & corr==max & residu!=11,]$corr,type='b',
...
#### Scatter plot
#png("/scratch/mbouseba/Documents/PhD/PLS/Etude PLS extreme/RICA/Inverse_yield/Quantiles/SPh15.png", width = 6, height = 5, units='in', res = 300,bg = "transparent")
...
#### Scatter plot
#png("/scratch/mbouseba/Documents/PhD/PLS/Etude PLS extreme/RICA/Inverse_yield/G/Gh15.png", width = 6, height = 5, units='in', res = 300,bg = "transparent")
